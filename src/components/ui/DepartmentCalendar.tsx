...
const DepartmentCalendar: React.FC = () => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm w-full h-full flex flex-col">
      {/* Header */}
      <div className="px-2 sm:px-3 md:px-5 py-2 sm:py-2.5 md:py-3 flex items-center justify-between flex-shrink-0">
        <h2 className="text-sm sm:text-base md:text-lg font-bold text-gray-900 truncate">Department Calendar</h2>
        <div className="flex items-center gap-0.5 sm:gap-1 md:gap-2">
          ...
          <span className="text-xs sm:text-sm font-semibold text-gray-900 w-20 sm:w-24 md:w-32 text-center truncate">
            {monthName}
          </span>
          <button
            onClick={nextMonth}
      ...
      {/* Calendar */}
      <div className="flex-1 overflow-hidden flex flex-col px-1.5 sm:px-2 md:px-5 py-2 sm:py-2.5 md:py-3">
          ...
          <div className="grid grid-cols-7 gap-0.5 sm:gap-1 md:gap-2 mb-1 sm:mb-2">
            {dayNames.map((day) => (
              ...
              >
                ...
                <span className="sm:hidden text-xs font-bold">{day.slice(0, 1)}</span>
              </div>
            ))}
          </div>

          {/* Calendar grid */}
          <div className="grid grid-cols-7 gap-0.5 sm:gap-1 md:gap-2 auto-rows-fr flex-1">
            {/* Empty days before month starts */}
            {emptyDays.map((_, i) => (
              <div key={`empty-${i}`} className="bg-gray-50 rounded-lg min-h-[30px] sm:min-h-[50px] md:min-h-[60px]" />
            ))}
            ...
            {daysArray.map((day, index) => {
              ...
              return (
                <div
                  key={day}
                  className="relative min-h-[30px] sm:min-h-[50px] md:min-h-[60px]"
                  onMouseEnter={() => setHoveredDay(day)}
                  onMouseLeave={() => setHoveredDay(null)}
                >
                  <div
                    className={`border border-gray-200 rounded-lg p-1 sm:p-2 text-center flex flex-col transition-all duration-200 h-full ${
                      dayEvents.length > 0
                  ...
                  >
                    ...
                    {dayEvents.length > 0 && (
                      <div className="flex flex-col gap-0.5 flex-1 justify-start">
                        <div className="text-xs font-medium text-indigo-600 truncate">
                          {dayEvents[0].title}
                        </div>
                      </div>
                    )}
                  ...
                  {isHovered && dayEvents.length > 0 && (
                    <div 
                      className={`absolute z-50 bg-white border border-gray-200 rounded-lg shadow-xl p-3 w-56 ${
                      ...
                      <div className="space-y-2">
                        {dayEvents.map((event, i) => (
                          <div key={i} className="flex items-start gap-2">
                            ...
                            <div className="text-xs text-gray-700">{event.title}</div>
                          </div>
                        ))}
                      </div>
                    </div>
